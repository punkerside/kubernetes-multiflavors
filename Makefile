...
base:
	...
	@docker build -t ${PROJECT}-${SERVICE}-java:build --build-arg IMG=${PROJECT}-${SERVICE}-base:latest -f docker/Dockerfile.java --target build .
...
build:
	...
	@docker run --rm -u ${DOCKER_UID}:${DOCKER_GID} -v ${PWD}/passwd:/etc/passwd:ro -v ${PWD}/app/product:/app ${PROJECT}-${SERVICE}-java:build
...
release:
	...
	@docker build -t ${PROJECT}-${SERVICE}-java:latest --build-arg IMG=${PROJECT}-${SERVICE}-base:latest -f docker/Dockerfile.java --target latest .
